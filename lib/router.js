...
module.exports = function (db, app, passport, appSecret) {
	...
	app.get('/approval', auth_handler, function(req,res) {
		db.sequelize.query("SELECT username, firstname, lastname, email, location, instruments, bio FROM approvals INNER JOIN app_users ON approved_user = username WHERE approving_user ='" + req.body.username + "';"
			).then (function(user) {
		 	res.json(user[0]);	
		})
	})
