...
module.exports = function (db, app, passport, appSecret) {
	...
	app.get('/unseen_user', auth_handler, function(req,res) {
		db.sequelize.query("SELECT username FROM app_users LEFT OUTER JOIN(SELECT rejected_user AS option FROM rejections WHERE rejecting_user=" + "'" + req.user.username + "'" + " UNION ALL SELECT approved_user AS option FROM approvals WHERE approving_user=" + "'" + req.user.username + "'" + ") as people_union ON username = option WHERE option IS NULL AND username!=" + "'" + req.user.username + "'" + ";"
		).then(function(data){
			res.json(data[0]);
		});
	});
